...
var Page = function Page() {
  ...
  this.getText = async function (page, ele) {
    await this.waitForElement(page, ele);
    return await page.$eval(ele, function (elem) {
  ...
  this.getElementCount = async function (page, ele) {
    await this.waitForElement(page, ele);
    return await page.$$eval(ele, function (elems) {
      return elems.length;
    });
  };
  ...
  this.isElementVisible = async function (page, ele) {
    var visibility = true;
    await page.waitForSelector(ele, { visible: true, timeout: 3000 }).catch(function () {
      visibility = false;
    });
    return visibility;
  };
  ...
  this.isXapthVisible = async function (page, xpath) {
    var visibility = true;
    await page.waitForXPath(xpath, { visible: true, timeout: 3000 }).catch(function () {
      visibility = false;
    });
    return visibility;
  };
  ...
  this.waitForText = async function (page, ele, text) {
    await this.waitForElement(page, ele);
    await page.waitForFunction(function (ele, text) {
      return document.querySelector(ele).innerText.includes(text);
    }, {}, ele, text);
  };
